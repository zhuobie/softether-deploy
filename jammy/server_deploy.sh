...
uninstall() {
    ...
    if [[ -f "/etc/netplan/01-softether-config.yaml" ]]; then
        rm -rf /etc/netplan/01-softether-config.yaml
        netplan apply
    fi
...
install() {
    ...
    if [[ "$secure_nat" = "secure_nat_yes" ]]; then
        ...
        echo "network:" >> /etc/netplan/01-softether-config.yaml
        echo "  ethernets:" >> /etc/netplan/01-softether-config.yaml
        echo "    vpn_vpn:" >> /etc/netplan/01-softether-config.yaml
        echo "      dhcp4: no" >> /etc/netplan/01-softether-config.yaml
        echo "      dhcp6: no" >> /etc/netplan/01-softether-config.yaml
        echo "      addresses:" >> /etc/netplan/01-softether-config.yaml
        echo "        - $ip_server/24" >> /etc/netplan/01-softether-config.yaml
        echo "  version: 2" >> /etc/netplan/01-softether-config.yaml
        netplan apply
...
status() {
    echo "=============================="
    if netstat -anp | grep $vpn_server_port > /dev/null 2>&1; then
